...
public class ConductorService : IConductor
    {
        ...
        public async Task<object> Send(BaseRequest request, CancellationToken cancellationToken = default)
        {
            ...
            var pipelineExecutor = _serviceProvider.GetService<Pipeline.IPipelineExecutor>();
        ...
        private async Task ValidateRequest(BaseRequest request, ValidateAttribute validateAttribute, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                if (autoValidator != null)
                {
                    ...
                    {
                        var validationTask = (Task<Conductor.Attributes.ValidationResult>)validateMethod.Invoke(autoValidator, [request, cancellationToken])!;
                ...
                if (!string.IsNullOrEmpty(validateAttribute.ValidatorType))
                {
                    ...
                    if (validatorType != null)
                    {
                        ...
                        if (validator != null)
                        {
                            ...
                            if (validateMethod != null)
                            {
                                var validationTask = (Task<Conductor.Attributes.ValidationResult>)validateMethod.Invoke(validator, new object[] { request, CancellationToken.None })!;
                                var validationResult = await validationTask;
                ...
                if (!Validator.TryValidateObject(request, validationContext, validationResults, true))
                {
                    ...
                    {
                        var errors = validationResults.Select(vr => new Conductor.Attributes.ValidationError(
                            vr.MemberNames.FirstOrDefault() ?? "",
                            vr.ErrorMessage ?? "",
                            "DataAnnotation")).ToArray();

                        throw new ConductorValidationException(Conductor.Attributes.ValidationResult.Failure(errors));
                    }
                }
            }
        ...
        private async Task ValidateResponse(object response, ValidateAttribute validateAttribute)
        {
            try
            {
                ...
                if (autoValidator != null)
                {
                    ...
                    if (validateMethod != null)
                    {
                        var validationTask = (Task<Conductor.Attributes.ValidationResult>)validateMethod.Invoke(autoValidator, new object[] { response, CancellationToken.None })!;
                        var validationResult = await validationTask;
                ...
                if (!string.IsNullOrEmpty(validateAttribute.ValidatorType))
                {
                    ...
                    if (validatorType != null)
                    {
                        ...
                        if (validator != null)
                        {
                            ...
                            if (validateMethod != null)
                            {
                                var validationTask = (Task<Conductor.Attributes.ValidationResult>)validateMethod.Invoke(validator, new object[] { response, CancellationToken.None })!;
                                var validationResult = await validationTask;
        ...
        private async Task<object> ExecuteWithTransactionAsync(BaseRequest request, Type handlerType, MethodInfo method, TransactionAttribute transactionAttribute, CancellationToken cancellationToken = default)
        {
            ...
            using var transactionScope = new Conductor.Attributes.TransactionScope(transactionAttribute);
            try
            {
                using var scope = _serviceScopeFactory.CreateScope();
                var handlerInstance = scope.ServiceProvider.GetRequiredService(handlerType);
